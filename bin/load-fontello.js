...
compressing.zip.uncompress(zipPath, uncompressDirPath).then(
  () => {
    ...
    fonts.forEach((font) => {
      fs.copyFileSync(path.join(filePath, 'font', font), path.join(targetPath, 'font', font));
      console.log(`Copy ${font} success`);
    });
    const config = JSON.parse(fs.readFileSync(path.join(filePath, 'config.json'), 'utf-8'));
    config.glyphs = config.glyphs.filter((glyph) => glyph.selected !== false);
    config['k-id'] = id;
    fs.writeFileSync(path.join(targetPath, 'config.json'), JSON.stringify(config, null, 2));

    console.log('Load config.json success');

    fs.writeFileSync(
      path.join(targetPath, 'font-defines.scss'),
      `@font-face {
